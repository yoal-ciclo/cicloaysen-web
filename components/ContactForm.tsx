...
export default function ContactForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setState('sending')
    try {
      const res = await fetch('https://formsubmit.co/ajax/contacto@cicloaysen.com', {
        method: 'POST',
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      {state === 'error' && (
        <p className="text-red-500 text-sm bg-red-50 border border-red-200 rounded-lg px-4 py-3">
          Hubo un error al enviar el mensaje. Por favor intenta nuevamente o escríbenos directamente a contacto@cicloaysen.com
        </p>
      )}
